...
UNION ALL SELECT 'member_mismatches', COUNT(*) FROM invoices i LEFT JOIN member_subscriptions ms ON ms.id = i.subscription_id LEFT JOIN subscriptions s ON s.id = i.subscription_v2_id WHERE (i.subscription_id IS NOT NULL AND (ms.id IS NULL OR i.member_id <> ms.member_id)) OR (i.subscription_v2_id IS NOT NULL AND (s.id IS NULL OR i.member_id <> s.holder_member_id))
UNION ALL SELECT 'financial_field_issues', COUNT(*) FROM invoices i WHERE (i.subscription_id IS NOT NULL OR i.subscription_v2_id IS NOT NULL) AND (i.subtotal < 0 OR i.tax_amount < 0 OR i.total < 0 OR ABS(i.total - (i.subtotal + i.tax_amount)) > 0.01 OR TRIM(COALESCE(i.currency, '')) = '' OR LOWER(TRIM(i.status)) NOT IN ('draft','issued','pending','partial','paid','overdue','unpaid','void','waived','cancelled','refunded'))
`;
